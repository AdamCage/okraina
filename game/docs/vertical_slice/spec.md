...
## Delta — E3 планировки (increment)

Живой контракт дельты: [`../run_floors/spec.md`](../run_floors/spec.md).
В этот файл он не помещается: восемь пресетов, веса зон, второй враг, seed.

### Меняется

- Длина забега 4–6 этажей от seed, не фиксированные три
- Этаж берёт id из списка пресетов, а не ветку `match` по номеру
- Второй враг со своей подготовкой

### Не меняется

- Находка в начале этажа, boon, смерть, выход, тон, запрет AUTHOR TRUTH
- Отпечаток этажа начинается с `F<номер>:`
- Это не генератор комнат и не дисковый save
- Старая дельта «≥3 различных этажей» не отменяет повтор id: при длине 6 алгоритм может повторить пресет смещения, когда пул зоны кончился
